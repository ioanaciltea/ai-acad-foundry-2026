# Libra Assist Backend
 
The backend for **Libra Assist**, a RAG-powered conversational assistant designed for Libra Internet Bank to help customers and bank staff navigate mortgage lending guidelines. It features dynamic chunking, deterministic vector storage in **Qdrant** (UUID5 deduplication), multi-agent personas (e.g. Mortgage Master), Text-to-Speech via Azure AI Speech, and strict Microsoft Entra ID authentication.
...
POST /tools/speak        # text → WAV (Azure AI Speech; uses Entra ID auth, needs AZURE_SPEECH_ENDPOINT & AZURE_SPEECH_RESOURCE_ID)
POST /tools/transcribe   # upload a WAV → text (NOTE: Currently broken, expects legacy API keys)
```
...
3. **Auth, keyless (recommended, local runs):** keep `AZURE_AI_AUTH=identity`, then
   `az login` once in your terminal ([install the CLI](How to install the Azure CLI)
...
1. Download LM Studio from [lmstudio.ai](LM Studio Bionic - Agent for Work and Code) and install.
